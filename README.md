...
TalentHub uses Jest for unit and integration testing. To run the tests:
...
Generate a coverage report to see how much of the codebase is tested:

```bash
npm run test:coverage
```

This will generate a detailed report in the console and create a `coverage` directory with an HTML report you can view in your browser.

### Writing New Tests

Test files are organized in the `tests` directory, mirroring the structure of the `src` directory:
- `tests/routes` - API endpoint tests
- `tests/services` - Service layer tests
- `tests/utils` - Utility function tests

Test fixtures and mock data are available in the `tests/fixtures` directory.

## Additional Information

- WebSocket functionality for real-time messaging is available at `/ws`.
- For local development with Stripe, use the Stripe CLI to forward webhook events.
